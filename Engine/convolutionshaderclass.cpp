...
bool ConvolutionShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	D3D11_BUFFER_DESC matrixBufferDesc, convolutionBufferDesc, screenSizeBufferDesc;
	...
	convolutionBufferDesc.ByteWidth = sizeof(ConvolutionBufferType);
...
void ConvolutionShaderClass::ShutdownShader()
{
	...
	if (m_convolutionBuffer)
	{
		m_convolutionBuffer->Release();
													...
													D3DXMATRIX projectionMatrix, D3DXMATRIX convolutionMatrix, ID3D11ShaderResourceView* texture, float screenWidth, float screenHeight)
{
	...
	// Get a pointer to the data in the constant buffer.
	ConvolutionBufferType *dataPtr3 = (ConvolutionBufferType*)mappedResource.pData;

	// Copy the matrices into the constant buffer.
	dataPtr3->matrix = convolutionMatrix;
	...
	// Now set the constant buffer in the vertex shader with the updated values.
	deviceContext->PSSetConstantBuffers(bufferNumber, 1, &m_convolutionBuffer);
